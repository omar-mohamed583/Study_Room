...
const mergeConfigs = (...configs: any) =>
  configs.reduce((acc: any, c: any) => ({ ...acc, ...c }), {});

// FIX: `position` is a key ('top' | 'bottom' | 'left' | 'right'), not the whole
// direction map. The old signature typed it as the map itself, which made
// `({...})[position]` an invalid index type in TypeScript. Pulling the map out
// into a typed Record and indexing a plain `string` fixes it.
